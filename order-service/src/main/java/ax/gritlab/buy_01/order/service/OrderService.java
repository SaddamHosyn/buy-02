...
public class OrderService {
    ...
    public List<Order> getKeyOrdersForBuyer(String buyerId) {
        ...
        return orderRepository
                .findByBuyerIdAndNotRemoved(buyerId, org.springframework.data.domain.PageRequest.of(0, 20))
                .getContent();
    }
    ...
    public List<Order> getOrdersForSeller(String sellerId) {
        ...
        return orderRepository
                .findBySellerIdInSellerIds(sellerId, org.springframework.data.domain.PageRequest.of(0, 20))
                .getContent();
    }
    ...
    public BuyerStats getBuyerStats(String buyerId) {
        ...
        orders.stream()
                ...
                .forEach(item -> productCounts.merge(item.getProductName(), item.getQuantity(), Integer::sum));
        ...
        // Calculate top categories (Mocked for now as we don't have category in
        // OrderItem yet)
        List<BuyerStats.CategoryPercentage> topCategories = List.of(
                new BuyerStats.CategoryPercentage("Electronics", 60.0),
                new BuyerStats.CategoryPercentage("Accessories", 30.0),
                new BuyerStats.CategoryPercentage("Other", 10.0));
    ...
    public SellerStats getSellerStats(String sellerId) {
        ...
        for (OrderItem item : sellerItems) {
            productRevenue.merge(item.getProductName(), item.getPriceAtPurchase() * item.getQuantity(), Double::sum);
        ...
        List<SellerStats.ProductRevenue> bestSelling = productRevenue.entrySet().stream()
                .map(e -> new SellerStats.ProductRevenue(e.getKey(), e.getValue(), productUnits.get(e.getKey())))
