...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected boolean shouldNotFilter(HttpServletRequest request) throws ServletException {
        ...
        // Skip JWT filter for health check and public endpoints
        return path.contains("/api/orders/health") ||
               path.contains("/api/orders/verify/") ||
    ...
    protected void doFilterInternal(
        ...
        if (SecurityContextHolder.getContext().getAuthentication() == null && jwtService.isTokenValid(jwt)) {
            ...
            Role role = authorities.stream()
                    ...
                    .filter(auth -> auth.equals("CLIENT") || auth.equals("SELLER"))
                    .findFirst()
                    .map(Role::valueOf)
                    .orElse(Role.CLIENT);

            // Create UserDetails object from token claims
