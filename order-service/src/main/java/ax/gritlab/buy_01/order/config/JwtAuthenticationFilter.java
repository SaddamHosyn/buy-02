...
import ax.gritlab.buy_01.order.service.JwtService;
...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected boolean shouldNotFilter(HttpServletRequest request) throws ServletException {
        ...
        // Skip JWT filter for health and verification endpoints
        return path.contains("/health") || path.contains("/verify/");
    }
    ...
    protected void doFilterInternal(
        ...
        if (SecurityContextHolder.getContext().getAuthentication() == null && jwtService.isTokenValid(jwt)) {
            ...
            Role role = Role.CLIENT; // default
            if (!authorities.isEmpty()) {
                String roleName = authorities.get(0).getAuthority();
                role = ax.gritlab.buy_01.order.model.Role.valueOf(roleName);
            }
